...
# ── 3. Function to run attack with a given clip norm ──────────────────────
def run_attack(clip_norm):
    ...
    # Apply gradient clipping defence
    if clip_norm > 0:
        clipped_grads = [tf.clip_by_norm(g, clip_norm) for g in real_grads]
        real_grads = clipped_grads
...
# ── 4. Run for all clip norms and print comparison table ──────────────────
print("Real data (first 6 features):", X_secret.numpy()[0][:6])
print("\n" + "="*55)
print(f"{'Clip Norm':<20} {'Reconstruction MSE':<20} {'Privacy'}")
print("="*55)

clip_norms = [0.0, 10.0, 1.0, 0.5, 0.1]
...
print("Lower clip norm = more aggressive clipping = stronger privacy")